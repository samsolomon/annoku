(function () {
  ...
  styleEl.textContent = [
    ...
    "  --relay-text-secondary: rgba(255, 255, 255, 0.55);",
    ...
    "  --relay-bg: rgba(255, 255, 255, 0.85);",
    "  --relay-bg-solid: #f0f0f0;",
    "  --relay-text: rgba(0, 0, 0, 0.88);",
    "  --relay-text-secondary: rgba(0, 0, 0, 0.60);",
    ...
    "  background: var(--relay-bg); backdrop-filter: blur(4px) saturate(1.6); -webkit-backdrop-filter: blur(4px) saturate(1.6);",
  ...
  function fetchAnnotations() {
    return fetch(getApiBase() + "/annotations").then(function (r) {
      return r.json();
    });
  }
  ...
  shortcuts.forEach(function (s) {
    ...
    tdKeys.innerHTML = s[1] ? "<kbd>" + s[0] + "</kbd> + <kbd>" + s[1] + "</kbd>" : "<kbd>" + s[0] + "</kbd>";
  ...
  function showEditPopover(annotation, pinEl) {
    ...
    saveBtn.addEventListener("click", function (e) {
      ...
      if (!text) return;
      saveBtn.disabled = true;
      updateAnnotation(annotation.id, { text: text })
        .then(function () {
          closePopover();
          refreshAnnotations();
        })
  ...
  function showMultiCreatePopover(elements, anchorRect, releasePoint) {
    ...
    saveBtn.addEventListener("click", function (e) {
      ...
      createAnnotation({
        ...
        .catch(function (err) {
          console.error("Multi-annotation save failed:", err);
          saveBtn.disabled = false;
        });
    });
